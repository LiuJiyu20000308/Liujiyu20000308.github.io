...
- **线程安全**：共享同一个控制块的不同 `shared_ptr` 实例可以被多个线程并发操作；具体边界见 3.7。
...
##### 3.6 **`shared_ptr` 的析构过程**

`shared_ptr` 的具体数据结构和函数实现由标准库决定，C++ 标准只规定它的行为。常见实现中，一个 `shared_ptr` 通常保存两个关键状态：

- **存储指针**：`get()` 返回、`operator*` 解引用的指针；
- **控制块指针**：指向控制块。控制块保存强引用计数、弱引用计数、删除器以及分配器等信息。

存储指针不一定就是最终被删除的指针，例如别名构造（aliasing constructor）可以让二者不同。因此，最后释放资源时调用的是控制块中保存的删除器，而不是简单地对 `get()` 执行 `delete`。

下面是一个忽略继承、异常处理和平台优化后的伪实现。许多实现会让弱引用计数额外包含一个“强引用仍存在”的隐式弱引用：

```cpp
struct ControlBlock {
    std::atomic<long> strong_count; // shared_ptr 的数量
    std::atomic<long> weak_count;   // weak_ptr 的数量 + 隐式弱引用

    void dispose(); // 调用删除器，销毁被管理对象
    void destroy(); // 释放控制块自身
};

void release_shared(ControlBlock* cb) noexcept {
    if (cb == nullptr) {
        return;
    }

    // 原子减一；只有观察到 1 -> 0 的线程负责销毁对象
...
shared_ptr<T>::~shared_ptr() noexcept {
    release_shared(control_block_);
}
```

因此，一个非空 `shared_ptr` 析构时大致经历以下步骤：

1. 原子地将控制块中的强引用计数减一。
2. 如果减一后仍大于 0，说明还有其他 `shared_ptr` 拥有对象，本次析构到此结束。
3. 如果当前线程使强引用计数从 1 变为 0，它就调用控制块中的删除器，销毁被管理对象。删除器可能是 `delete`，也可能是用户提供的自定义删除器。
4. 对象销毁后再减少隐式弱引用；只有弱引用计数也归零时，才释放控制块。

`std::make_shared` 通常会把对象和控制块放在同一次内存分配中，但二者的生命周期仍然分开：强引用归零时先执行对象的析构函数，弱引用归零时才真正回收包含控制块的那块内存。这也是对象已经销毁后，`weak_ptr::expired()` 和 `weak_ptr::lock()` 仍能安全判断对象状态的原因。

> 注意：上面的代码用于解释原理，并不是标准规定或某个标准库源码的逐行复制。不同实现可能使用专门的原子指令、内存屏障或其他计数布局。

##### 3.7 **为什么说 `shared_ptr` 是线程安全的**

`shared_ptr` 的线程安全来自**共享控制块中的计数操作具有原子性和必要的同步保证**。多个线程各自持有不同的 `shared_ptr` 实例，但这些实例共享同一个控制块时，可以并发地执行拷贝、赋值和析构：

- 拷贝 `shared_ptr` 时，强引用计数被原子地增加；
- 析构或 `reset()` 时，强引用计数被原子地减少；
- 对于最后一次释放，原子的“减一并返回旧值”操作保证只有一个线程能观察到 `1 -> 0`，因此被管理对象只会被销毁一次；
- 控制块同样会在最后一个弱引用消失后只释放一次。

不过，“`shared_ptr` 是线程安全的”有严格的边界：

1. **不同的 `shared_ptr` 实例**可以在不同线程中并发修改，即使它们管理的是同一个对象。
2. **同一个 `shared_ptr` 实例**不能在没有同步的情况下被多个线程并发修改。例如，一个线程执行 `reset()`，另一个线程同时对同一个变量赋值，会产生数据竞争。C++11/14 中可使用互斥锁，或者始终通过 `std::atomic_load`、`std::atomic_store` 等针对 `shared_ptr` 的自由函数访问它；`std::atomic<std::shared_ptr<T>>` 是 C++20 才提供的。
3. **被管理对象本身不因此变得线程安全**。多个线程通过各自的 `shared_ptr` 同时修改对象，仍需要对象自己的锁、原子变量或其他同步机制。
4. `use_count()` 只是在调用瞬间取得的计数快照。其他线程可能马上改变计数，因此不要用它来判断“随后是否可以独占访问对象”。

可以把结论概括为：**控制块的生命周期管理是线程安全的；同一个智能指针变量的并发写入和所指对象的并发访问，不由 `shared_ptr` 自动保护。**
...
**`shared_ptr` 只保证共享控制块的生命周期管理是线程安全的，并不保证被管理对象本身是线程安全的。**
...
- 右值引用通过 `T&&` 定义，可以捕获右值。对于仍有名字的左值，可以使用 `std::move()` 将它显式转换为可被移动的右值。
...
`std::move` 定义在 `<utility>` 中。它本身既不搬运内存，也不保证一定调用移动构造函数；它只是把表达式无条件转换为右值（更准确地说是 xvalue），从而允许编译器选择接收方的移动构造函数或移动赋值运算符。
...
因此，真正执行资源转移的是目标类型的移动构造函数或移动赋值运算符，而不是 `std::move` 本身：

```cpp
std::string source = "a long string";
std::string target = std::move(source);
// std::move(source) 只负责转换值类别；
// std::string 的移动构造函数负责接管 source 的内部资源。
```

#### 5.2 **`std::move` 的常见使用场景**

##### 1. **转移只能移动、不能拷贝的资源所有权**

`std::unique_ptr`、`std::thread`、`std::future`、`std::promise` 和 `std::unique_lock` 等类型通常禁止拷贝，只允许移动：
...
// p1 已经不再拥有对象，p2 接管所有权
assert(p1 == nullptr);
```

`shared_ptr` 虽然可以拷贝，但移动时可以直接转移当前智能指针持有的所有权，通常不需要增加再减少引用计数：

```cpp
auto p1 = std::make_shared<int>(42);
auto p2 = std::move(p1);

assert(!p1);  // 移动后的 shared_ptr 为空
assert(p2);
```

##### 2. **把大型对象交给另一个对象或容器**

对于 `std::string`、`std::vector` 等持有动态资源的对象，确定不再需要原对象内容时，可以移动以避免深拷贝：
...
// name 仍然是合法对象，但其值处于“有效但未指定”的状态
```

只有在后续不再依赖原值时才应这样做。如果还要继续读取 `name` 的原内容，就不应该移动它。

##### 3. **实现类的移动构造函数和移动赋值运算符**

右值引用参数虽然类型是 `T&&`，但只要它有名字，它在函数体中就是左值。因此移动其成员时仍然需要 `std::move`：
...
移动构造函数尽量声明为 `noexcept`。例如，`std::vector` 扩容时通常只有在元素的移动构造函数不会抛出异常时，才会优先移动旧元素，否则可能为了异常安全而选择拷贝。

##### 4. **把对象交给明确接管所有权的函数**

如果函数参数按值接收资源，调用方可以用 `std::move` 明确表示“调用之后我不再需要这个对象”：
...
这样传入右值时可以一路移动；传入左值时只在进入函数时拷贝一次，随后再把参数移动到成员中。

##### 5. **在 C++14 Lambda 中转移捕获资源**

C++14 的初始化捕获可以把只能移动的对象交给 Lambda：

```cpp
auto p = std::make_unique<int>(42);

auto task = [value = std::move(p)] {
...
这种写法常用于异步任务、回调和线程函数，因为资源的生命周期会跟随 Lambda 对象。

#### 5.3 **使用 `std::move` 时的注意事项**

1. **移动后的对象仍然可以析构和重新赋值，但不要依赖其原值。**大多数标准库类型在移动后处于“有效但未指定”的状态；可以对它赋新值、调用 `clear()` 或让它正常析构。某些类型会提供更强的保证，例如移动后的 `unique_ptr` 和 `shared_ptr` 明确为空。

2. **不要随意对 `const` 对象使用 `std::move`。**移动操作通常需要修改源对象，而 `std::move` 不会去掉 `const`，所以这种情况往往仍然调用拷贝构造函数：

   ```cpp
   const std::string source = "hello";
   std::string target = std::move(source); // 通常仍然是拷贝
   ```

3. **返回局部变量时通常不要写 `std::move`。**编译器可以执行返回值优化；即使没有消除构造，局部变量在返回时也可以被隐式移动。显式添加 `std::move` 反而可能阻止 NRVO：

   ```cpp
   std::string make_name() {
       std::string name = "Alice";
       return name;             // 推荐：允许 NRVO 或隐式移动
       // return std::move(name); // 通常没有必要
   }
   ```

4. **泛型转发函数通常应该使用 `std::forward`，而不是无条件使用 `std::move`。**`std::move` 会把传入的左值也转换成右值；`std::forward` 则会保留调用者传入参数的原始值类别。

5. **类型没有移动操作时，写了 `std::move` 也可能发生拷贝。**右值同样可以绑定到 `const T&`，因此当移动构造函数不存在或不可用时，编译器可能选择拷贝构造函数。

> 另外，`<algorithm>` 中还有一个接收三个迭代器的 `std::move(first, last, destination)` 算法，用于逐个移动一个区间的元素。它与 `<utility>` 中接收单个对象、负责转换值类别的 `std::move(value)` 作用不同。
